...
func LamdaHandler(context context.Context, request events.APIGatewayV2HTTPRequest) (events.APIGatewayProxyResponse, error) {
	...
	// List Todo Items.
	if string(request.RequestContext.HTTP.Method) == "GET" && request.RequestContext.HTTP.Path == "/" {
	...
	// Get Todo Item by ID.
	if string(request.RequestContext.HTTP.Method) == "GET" && request.RequestContext.HTTP.Path == "/item" {
	...
	// Create Todo Item.
	if string(request.RequestContext.HTTP.Method) == "POST" && request.RequestContext.HTTP.Path == "/item" {
	...
	// Update Todo Item.
	if string(request.RequestContext.HTTP.Method) == "PUT" && request.RequestContext.HTTP.Path == "/item" {
		var params todoRequestParams
		if len(request.Body) <= 0 {
			return infra.APIResponse(400, domain.ErrInvalidParameters)
		}
	...
	// Delete Todo Item.
	if string(request.RequestContext.HTTP.Method) == "DELETE" && strings.HasPrefix(request.RawPath, "/item/") {
	...
	// Other
	body := struct{ Msg string }{
		Msg: "\"Http method or request path is not supported.\"",
	}
	response, _ = infra.APIResponse(400, body)
	return response, errors.New("http method  or request path is not supported")
}
